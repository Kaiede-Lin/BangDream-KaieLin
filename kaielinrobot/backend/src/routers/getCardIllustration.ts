...
router.post('/',
  [
    ...
    body('cardId').optional().custom((value) => typeof value === 'string' || Number.isInteger(value)),
  ...
  async (req: Request, res: Response) => {
    ...
    if (cardText && fuzzySearchResult) {
      return res.send(listToBase64(['text 与 fuzzySearchResult 不能同时存在']))
    }
    try {
...
async function commandGetCardIllustration(cardText: string | number | FuzzySearchResult): Promise<Array<Buffer | string>> {
  //console.log(cardText)
  var after_training = null
  let fuzzySearchResult: FuzzySearchResult
  var cardId = 0
  if (typeof cardText === 'number') {
    cardId = cardText
  }
