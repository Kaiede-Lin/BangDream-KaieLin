...
export class Cutoff {
    ...
    predictEP: number = 0;
    predictEP2: number = 0;
    predictConfidence: number = 0;
    predictRushScore: number = 0;
    predictMode: string = 'normal';
    predictRangeMin: number = 0;
    predictRangeMax: number = 0;
    startAt: number;
    startAtAll: number[];
    endAt: number;
    endAtAll: number[];
    ...
    async initFull() {
        if (this.isInitfull || this.isExist == false) return
        let cutoffData
        let pCutoffData
    ...
    predict(): number {
        ...
        try {
            var result = predict(cutoff_ts, start_ts, end_ts, this.rate, this.dailyIncrement)
        } catch (e) {
            console.log(e)
            this.predictEP = 0
            this.predictConfidence = 0
            this.predictRushScore = 0
            this.predictMode = 'error'
            this.predictRangeMin = 0
            this.predictRangeMax = 0
            return this.predictEP
        }
        this.predictEP = Math.floor(result.ep)
        this.predictConfidence = result.confidence ?? 0
        this.predictRushScore = result.rushScore ?? 0
        this.predictMode = result.mode ?? 'normal'
        this.predictRangeMin = Math.floor(result.rangeMin ?? this.predictEP)
        this.predictRangeMax = Math.floor(result.rangeMax ?? this.predictEP)
        return this.predictEP
    }
    predict2(): number {
    ...
    getPredictionHistory(): { time: number, ep: number, confidence?: number, rushScore?: number, mode?: string }[] {
        if (this.isExist == false || !this.cutoffs) return []
        const event = new Event(this.eventId)
        ...
        const history: { time: number, ep: number, confidence?: number, rushScore?: number, mode?: string }[] = []
        for (let i = 0; i < cutoff_ts.length; i++) {
            let result
            try {
                result = predict(cutoff_ts.slice(0, i + 1), start_ts, end_ts, this.rate, this.dailyIncrement)
            } catch (e) {
                continue
            }
            if (result && result.ep && !isNaN(result.ep) && result.ep !== 0) {
                history.push({
                    time: this.cutoffs[i].time,
                    ep: Math.floor(result.ep),
                    confidence: result.confidence ?? 0,
                    rushScore: result.rushScore ?? 0,
